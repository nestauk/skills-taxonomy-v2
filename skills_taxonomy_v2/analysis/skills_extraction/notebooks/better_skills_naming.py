...
def clean_cluster_descriptions(sentences_data): #remove job based stop words!
    ...
    work_stopwords = ['essential', 'requirement', 'degree', "responsibility", "duties", "responsibilities", "experienced", 'previous', 'andor', 'minimum', 'years']
...
def get_clean_ngrams(sentence_skills, ngram, min_count, threshold):
    ...
    clean_ngrams = [' '.join(OrderedDict((w,w) for w in ngrm.split()).keys()) for ngrm in clean_ngrams]

    #only return ngrams that are more than 1 word long 
    return [clean for clean in clean_ngrams if len(clean.split(' ')) > 1], cluster_descriptions


def get_skill_info(sentence_skills, sentence_embs, num_top_sent=2, ngram=4, min_count=1, threshold=0.15):
    """
    ...
    for cluster_num, cluster_data in tqdm(sentence_skills.groupby("Cluster number")):
        ...
        candidate_ngrams, cluster_descriptions = get_clean_ngrams(cluster_data, ngram, min_count, threshold)
        
        if len(candidate_ngrams) > 1: #if there are more than 1 candidate ngrams, skill cluster is labelled as the closest ngram embedding to the cluster mean embedding
            ...
            closest_ngram = candidate_ngrams[int(ngram_similarities.argsort()[0][::-1].tolist()[0:1][0])]
            
            skills_data[cluster_num] = {"Skills name": closest_ngram,
                                        "Name method": "phrases_embedding",
                                        'Skills name embed': candidate_ngrams_embeds[int(ngram_similarities.argsort()[0][::-1].tolist()[0:1][0])],
                                        "Examples": " ".join(
                    [cluster_text[i] for i in sent_similarities.argsort()[0][::-1].tolist()[0:num_top_sent]]
                ),
                "Texts": cluster_descriptions[cluster_num],
            }
                               
        else:
            print('no candidate ngrams') #if no candidate ngrams are generated, skill name is smallest skill description
            skills_data[cluster_num] = {"Skills name": min(cluster_descriptions[cluster_num], key=len),
                                        "Name method": "minimum description",
                                        'Skills name embed': bert_vectorizer.transform([min(cluster_descriptions[cluster_num], key=len)])[0],
                                        "Examples": " ".join(
                    [cluster_text[i] for i in sent_similarities.argsort()[0][::-1].tolist()[0:num_top_sent]]
                ),
                "Texts": cluster_descriptions[cluster_num],
            }   
        cand_ngrams.append(candidate_ngrams)
            
    print("--- %s seconds ---" % (time.time() - start_time))
            
    return skills_data, cand_ngrams
...
#random sample
k = 5
random_clusters = random.sample(list(set(sentence_skills["Cluster number"].tolist())), k)
sentence_skills_sample = sentence_skills[sentence_skills["Cluster number"].isin(random_clusters)]
...
# %%
named_skills, cand_ngrams = get_skill_info(sentence_skills_sample, sentence_embs)
...
cluster_counts = sentence_skills_sample.groupby('Cluster number').count().sort_values('description', ascending = False)
cluster_counts[cluster_counts['description'] > 10] 
cluster_counts[cluster_counts['description'] <= 5] 
cluster_counts['description'].plot.hist(bins = 15, range=[5,50]) #vast majority of clusters are quite small! 
...
candidate_phrases = list(set([phrase.text.strip() for phrase in doc._.phrases 
...
candidate_chunks = [noun.strip() for noun in noun_chunks if 2 <= len(noun.strip().split(' ')) <= 4]
...
def get_clean_ngrams_pytextrank(sentence_skills, min_ngram=2, max_ngram=3):
    ...
    for cluster_text in cluster_texts:
        ...
        clean_chunks = [' '.join(OrderedDict((w,w) for w in ngrm.split(' '))) for ngrm in chunks]
        candidate_chunks.append([chunk for chunk in clean_chunks if 
                                 min_ngram <= len(chunk.split(' ')) <= max_ngram])
...
def get_clean_ngrams_chunks(sentence_skills):
    ...
    nlp = spacy.load("en_core_web_sm")
    patterns = ["VERB NOUN", "NOUN VERB", "VERB ADJ NOUN", "VERB NOUN NOUN"] #experiment with this! 
    ...
    for cluster_text in cluster_texts:
        ...
        candidate_chunk = [[" ".join(result) for result in re.findall(pattern, text_pos)] 
                        for i, pattern in enumerate(re_patterns)]
        candidate_chunks.append(candidate_chunk)
    
    return list(set(itertools.chain(*list(itertools.chain.from_iterable(candidate_chunks))))), cluster_descriptions
...
for cluster_num, cluster_data in tqdm(sentence_skills_sample.groupby("Cluster number")):
    ...
    candidate_ngrams, cluster_descriptions = get_clean_ngrams_pytextrank(cluster_data, min_ngram=2, max_ngram=3)
    ...
    if len(candidate_ngrams) > 1: #if there are more than 1 candidate ngrams, skill cluster is labelled as the closest ngram embedding to the cluster mean embedding
        ...
        skills_data[cluster_num] = {"Skills name": closest_ngram,
                                    ...
                                    'Skills name embed': candidate_ngrams_embeds[int(ngram_similarities.argsort()[0][::-1].tolist()[0:1][0])],
                                    "Examples": " ".join(
                [cluster_text[i] for i in sent_similarities.argsort()[0][::-1].tolist()[0:2]]
            ),
            "Texts": cluster_descriptions[cluster_num],
        }                          
...
# %%
def merge_skill_clusters(named_skills, 
                         sentence_skills, 
                         candidate_ngrams, 
                         skill_name_sim_threshold = 0.9, 
                         centroid_threshold = 0.9):
    
    skill_name_sims = cosine_similarity(np.array([skill_data['Skills name embed']
                   for skill_clust, skill_data in named_skills.items()])) 
    
    candidate_phrases = [skill_data['Skills name'] for skill_clust, skill_data in named_skills.items()]
    ...
    duplicate_skills = [x for x in duplicate_skills if not any(set(x)<=set(y) 
                                                               for y in duplicate_skills if x is not y)]    
    ...
    for skill_ids in duplicate_skills:
        clust_centroids = [np.mean([np.array(embeds).astype("float32") for embeds 
                                in sentence_skills[sentence_skills['Cluster number'] == skill_id]['reduced_points_umap'].values], axis=0)
                              for skill_id in skill_ids]
        centroid_sims = cosine_similarity(clust_centroids)
        centroid_sims = centroid_sims[~np.eye(len(centroid_sims), dtype=bool)].reshape(len(centroid_sims), -1) #remove diagonal values of cosine sims to itself 
        # if ALL cluster centroids very close together, merge skills
        if all([all(list(sim > centroid_threshold)) == True for sim in centroid_sims]) == True:
            print(centroid_sims)
            
            phrase_similarities = cosine_similarity(
            np.mean(skill_name_sims, axis=0).reshape(1, -1), skill_name_sims
        )  
            merged_skills['_'.join([str(skill_id) for skill_id in skill_ids])] = {
                    'Skills name': candidate_phrases[int(phrase_similarities.argsort()[0][::-1].tolist()[0:1][0])], #name is phrase embed closest to phrase centroid
                    'Texts': list(itertools.chain(*[named_skills[skill_id]['Texts'] for skill_id in skill_ids]))
                }
    
    if merged_skills:
        flat_dup_skills = list(itertools.chain(*duplicate_skills))
        named_skills_no_dups = {key: named_skills[key] for key in named_skills if key not in flat_dup_skills}
...
# %%
merged_skills = merge_skill_clusters(named_skills, 
                                     sentence_skills_sample, 
                                     cand_ngrams,
                                     skill_name_sim_threshold = 0.2, 
                                     centroid_threshold = 0.2)
pd.DataFrame(merged_skills).T
