...
def get_word_stats(word):
    word_weights = [w['weight'] for s,e,w in list(net.edges(data = True)) if s==word or e==word]
...
y = [n[1] for n in node_stats.values()] # 1: mean, 2: median weight of all connections
...
p = figure(plot_width=500, plot_height=500,
        ...
        toolbar_location="below" ,x_range=(0, 300), y_range=(0, 6))
p.circle(x="x",y="y",radius=1, alpha=0.7,source=source,)
show(p)
...
# %%
words_frequency = pd.Series(list(chain(*lemma_sentence_words_clean))).value_counts() / len(lemma_sentence_words_clean)
...
def filter_extreme_results(results_dict, filter_top=0.1, filter_bottom=0.1):
    sorted_words = [k for k, v in sorted(results_dict.items(), key=lambda item: item[1])]
    return sorted_words[round(len(sorted_words)*filter_bottom):round(len(sorted_words)*(1-filter_top))]
...
keep_words = filter_extreme_results(results_dict, filter_top=filter_top, filter_bottom=filter_bottom)
...
viz.plot_network_clusters(net_filt, coms, pos, figsize=(15,15), node_size=50, plot_labels=False)
