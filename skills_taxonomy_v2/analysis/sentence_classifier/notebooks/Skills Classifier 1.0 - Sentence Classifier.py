...
def mask_job(
    text, spacy_ner_types=["DATE", "MONEY", "CARDINAL", "TIME", "ORDINAL", "QUANTITY"]
):
    doc = nlp(text)
    # '6 months' -> '######'
...
def remove_camelcase(text, skill_spans):
    ...
    change_points = [
        instance.start() + 1
        for instance in re.finditer(compiled_missing_space_pattern, text)
    ]
    for span_i, (span_s, span_e) in enumerate(skill_spans):
        ...
        skill_spans[span_i] = (
            span_s + num_changes_before_s,
            span_e + num_changes_before_e,
        )
...
plt.hist(
    [len(x) for i, x in enumerate(X) if y[i] == 0],
    30,
    color="red",
    alpha=0.7,
    density=True,
...
plt.hist(
    [len(x) for i, x in enumerate(X) if y[i] == 1],
    30,
    color="green",
    alpha=0.5,
    density=True,
...
need_cut = [
    ...
    if (bert_tokenizer.encode(x, return_tensors="pt")).shape[1] >= 510
]
print(
    f"{len(need_cut)} out of {len(X_train)} of the training data points needed cutting down so will lose information"
)
...
print(
    f"{len(need_cut)} out of {len(X_test)} of the test data points needed cutting down so will lose information"
)
...
df = pd.DataFrame(
    {
        ...
        "Correct prediction?": [
            "True" if a == p else "False" for a, p in zip(y_test, y_test_pred)
        ],
    }
)
...
def use_model(
    X_train_vect, y_train, X_test_vect, y_test, classfier_type, use_scaler=None
):
    if use_scaler:
        # Fit scaler
        scaler = MinMaxScaler()
...
for classfier_type in ["mnb", "svm", "log_reg"]:
    for use_scaler in [True, None]:
        if classfier_type == "mnb":
            use_scaler = True
        all_results[f"{classfier_type}_{str(use_scaler)}"] = use_model(
            X_train_vect,
            y_train,
            X_test_vect,
            y_test,
            classfier_type=classfier_type,
