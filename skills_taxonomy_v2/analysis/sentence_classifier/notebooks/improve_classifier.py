...
with open(
    "/Users/india.kerlenesta/Projects/skills-taxonomy-v2/inputs/new_training_data/training_data.pickle",
    "rb",
) as fp:
...
def preprocess_training_data(training):
    return [
        (re.sub(r"\d+[^\w]", " ", string[0]).lower().strip(), string[1])
...
no_skill_undersample = random.sample(
...
balanced_augment_embed = skills_augment_embed + [
    (train, label) for train, label in zip(X_train, y_train) if label == 0
]
...
# %%
oversample_undersample_training_syns = skills_augment + random.sample(
    [(train, label) for train, label in zip(X_train, y_train) if label == 0],
    len(skills_augment),
)
...
# %%
oversample_undersample_training_embeds = skills_augment_embed + random.sample(
    [(train, label) for train, label in zip(X_train, y_train) if label == 0],
...
for sentence, probability, test_label, predicted_label in zip(
    X_test, predicted_probabilities, y_test, predict
):
...
for key, value in different_splits.items():
    ...
    (
        ...
        different_splits[key]["X_train"],
        different_splits[key]["y_train"],
        different_splits[key]["X_test"],
        different_splits[key]["y_test"],
    )
    print("------------")
...
(
    ...
    different_splits["seed_22"]["X_train"],
    different_splits["seed_22"]["y_train"],
    different_splits["seed_22"]["X_test"],
    different_splits["seed_22"]["y_test"],
)
