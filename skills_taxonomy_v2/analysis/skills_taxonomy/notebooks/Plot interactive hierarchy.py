...
average_emb_clust = sentence_data.groupby("Cluster number")[['reduced_points x', 'reduced_points y']].mean().to_dict(orient='index')
average_emb_clust = {str(k): [v['reduced_points x'], v['reduced_points y']] for k, v in average_emb_clust.items()}


skill_hierarchy_df['Average reduced_points x'] = skill_hierarchy_df['Skill number'].apply(lambda x: average_emb_clust.get(str(x))[0])
skill_hierarchy_df['Average reduced_points y'] = skill_hierarchy_df['Skill number'].apply(lambda x: average_emb_clust.get(str(x))[1])
...
def filter_skills(skill_hierarchy_df, level_A, level_B, level_C):
    ...
    if level_A != 'All':
        skill_hierarchy_filt = skill_hierarchy_filt[skill_hierarchy_filt['Hierarchy level A']==level_A]
        if level_B != 'All':
            skill_hierarchy_filt = skill_hierarchy_filt[skill_hierarchy_filt['Hierarchy level B']==level_B]
            if level_C != 'All':
                skill_hierarchy_filt = skill_hierarchy_filt[skill_hierarchy_filt['Hierarchy level C']==level_C]
...
def update_colours(skill_hierarchy_df, left_gp, col_by, radius_size, alpha):
    ...
    left = left_gp.circle(
        ...
        radius=radius_size,
        alpha=alpha,
        color={"field": f"Hierarchy level {col_by}",
...
hover = HoverTool(
        tooltips=[
            ...
            ("Number of sentences that created skill","@{Number of sentences that created skill}"),
...
left = update_colours(skill_hierarchy_df, left_gp, col_by='A', radius_size=0.1, alpha=0.5)
    
p = gridplot([[left_gp]])
show(p, notebook_handle=True)
interact(update, col_by=['A','B','C'],
