...
from bokeh.io import output_file, show, push_notebook, output_notebook, reset_output, save

from bokeh.plotting import figure, from_networkx, ColumnDataSource, output_file, show, gridplot
...
# %%
job_id_levels['Hierarchy level A normalised'] = job_id_levels['Hierarchy level A']/job_id_levels['sentence id']
job_id_levels['Hierarchy level B normalised'] = job_id_levels['Hierarchy level B']/job_id_levels['sentence id']
job_id_levels['Hierarchy level C normalised'] = job_id_levels['Hierarchy level C']/job_id_levels['sentence id']
...
plt.savefig('outputs/skills_taxonomy/figures/2021.09.06/evaluate_mean_nums.pdf',bbox_inches='tight')
...
job_id_levels[job_id_levels['sentence id']>=min_num_sent]['Hierarchy level A normalised'].plot.hist(
    ax=axes[0], color=nesta_orange,
    title="Number of unique level A skill\ngroups per job id (normalised)")
job_id_levels[job_id_levels['sentence id']>=min_num_sent]['Hierarchy level B normalised'].plot.hist(
    ax=axes[1], color=nesta_orange,
    title="Number of unique level B skill\ngroups per job id (normalised)")
job_id_levels[job_id_levels['sentence id']>=min_num_sent]['Hierarchy level C normalised'].plot.hist(
    ax=axes[2], color=nesta_orange,
    title="Number of unique level C skill\ngroups per job id (normalised)")

plt.tight_layout()


plt.savefig('outputs/skills_taxonomy/figures/2021.09.06/evaluate_nums_normalised.pdf',bbox_inches='tight')
...
job_id_levels[job_id_levels['sentence id']>=min_num_sent]['Hierarchy level A'].plot.hist(
    ax=axes[0], color=nesta_orange,
    title="Number of unique level A skill\ngroups per job id", bins=6)
job_id_levels[job_id_levels['sentence id']>=min_num_sent]['Hierarchy level B'].plot.hist(
    ax=axes[1], color=nesta_orange,
    title="Number of unique level B skill\ngroups per job id")
job_id_levels[job_id_levels['sentence id']>=min_num_sent]['Hierarchy level C'].plot.hist(
    ax=axes[2], color=nesta_orange,
    title="Number of unique level C skill\ngroups per job id")

plt.tight_layout()

plt.savefig('outputs/skills_taxonomy/figures/2021.09.06/evaluate_nums_raw.pdf',bbox_inches='tight')
...
job_id_levels[job_id_levels['sentence id']==min_num_sent]['Hierarchy level A'].plot.hist(
    ax=axes[0], color=nesta_orange,
    title="Number of unique level A skill\ngroups per job id", bins=min_num_sent)
job_id_levels[job_id_levels['sentence id']==min_num_sent]['Hierarchy level B'].plot.hist(
    ax=axes[1], color=nesta_orange,
    title="Number of unique level B skill\ngroups per job id", bins=min_num_sent)
job_id_levels[job_id_levels['sentence id']==min_num_sent]['Hierarchy level C'].plot.hist(
...
plt.savefig('outputs/skills_taxonomy/figures/2021.09.06/evaluate_nums_6.pdf',bbox_inches='tight')
...
# %%
job_titles = load_s3_data(s3, bucket_name,
...
# %%
sentence_data['Job title'] = sentence_data['job id'].apply(lambda x: job_titles.get(x,[None])[0])
sentence_data['Organization industry'] = sentence_data['job id'].apply(lambda x: job_titles.get(x,[None,None])[1])

# %%
print(f"There are {sentence_data['Job title'].nunique()} unique job titles")
print(f"There are {sentence_data['Organization industry'].nunique()} unique organization industries")

# %%
print(f"{round(sum(sentence_data['Job title'].notnull())*100/len(sentence_data),3)}% skill sentences have job title data")
print(f"{round(sum(sentence_data['Organization industry'].notnull())*100/len(sentence_data),3)}% skill sentences have organization industry data")
...
# %%
leve_a_group['Cluster number'].apply(lambda x: skills_data[str(x)]['Skills name']).value_counts()[0:5].index
...
for level_a_name, leve_a_group in sentence_data.groupby(['Hierarchy level A']):
    ...
    lev_a_top_jobs.append({
        ...
        'Top 5 most common skills':list(leve_a_group['Cluster number'].apply(lambda x: skills_data[str(x)]['Skills name']).value_counts()[0:5].index),
        'Top 10 job titles': [f"{j} ({n})" for j,n in list(zip(job_titles.index[0:10], job_titles[0:10]))],
        'Top 10 organization industries': [f"{j} ({n})" for j,n in list(zip(org_inds.index[0:10], org_inds[0:10]))]
    })

# %%
pd.DataFrame(lev_a_top_jobs).to_csv("outputs/skills_taxonomy/evaluation/top_10_jobs_levela.csv")
...
for level_b_name, leve_b_group in sentence_data.groupby(['Hierarchy level B']):
    ...
    lev_b_top_jobs.append({
        ...
        'Top 5 most common skills':list(leve_b_group['Cluster number'].apply(lambda x: skills_data[str(x)]['Skills name']).value_counts()[0:5].index),
        'Top 10 job titles': [f"{j} ({n})" for j,n in list(zip(job_titles.index[0:10], job_titles[0:10]))],
        'Top 10 organization industries': [f"{j} ({n})" for j,n in list(zip(org_inds.index[0:10], org_inds[0:10]))]
    })

# %%
pd.DataFrame(lev_b_top_jobs).to_csv("outputs/skills_taxonomy/evaluation/top_10_jobs_levelb.csv")
...
for level_c_name, leve_c_group in sentence_data.groupby(['Hierarchy level C']):
    ...
    lev_c_top_jobs.append({
        ...
        'Top 5 most common skills':list(leve_c_group['Cluster number'].apply(lambda x: skills_data[str(x)]['Skills name']).value_counts()[0:5].index),
        'Top 10 job titles': [f"{j} ({n})" for j,n in list(zip(job_titles.index[0:10], job_titles[0:10]))],
        'Top 10 organization industries': [f"{j} ({n})" for j,n in list(zip(org_inds.index[0:10], org_inds[0:10]))]
    })

# %%
pd.DataFrame(lev_c_top_jobs).to_csv("outputs/skills_taxonomy/evaluation/top_10_jobs_levelc.csv")
...
# %%
sentence_data['Skill name'] = sentence_data['Cluster number'].apply(lambda x: skills_data[str(x)]['Skills name'])
sentence_data['Hierarchy level B name'] = sentence_data['Hierarchy level B'].apply(lambda x: lev_b_name_dict[str(x)])
sentence_data['Hierarchy level C name'] = sentence_data['Hierarchy level C'].apply(lambda x: lev_c_name_dict[str(x)])

# %%
per_top_job_levels = []
for job_title in top_jobs:
    job_df = sentence_data[sentence_data['Job title']==job_title]
    levas = round(job_df['Hierarchy level A name'].value_counts(sort=True)/len(job_df),3)
    levbs = round(job_df['Hierarchy level B name'].value_counts(sort=True)/len(job_df),3)
    levcs = round(job_df['Hierarchy level C name'].value_counts(sort=True)/len(job_df),3)
    ...
    per_top_job_levels.append(
        {
            ...
            'Most common level A skill groups': [f"{j} ({n})" for j,n in list(zip(levas.index[0:1], levas[0:1]))],
            '5 most common level B skill groups': [f"{j} ({n})" for j,n in list(zip(levbs.index[0:5], levbs[0:5]))],
            '5 most common level C skill groups': [f"{j} ({n})" for j,n in list(zip(levcs.index[0:5], levcs[0:5]))],
            '5 most common skills': [f"{j} ({n})" for j,n in list(zip(skills_d.index[0:10], skills_d[0:5]))]
...
# %%
pd.DataFrame(per_top_job_levels).to_csv("outputs/skills_taxonomy/evaluation/jobs_top_skill_groups.csv")
