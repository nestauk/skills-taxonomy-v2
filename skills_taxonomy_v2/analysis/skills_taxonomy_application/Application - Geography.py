...
# %%
jobs_with_nuts_codes = load_s3_data(s3, bucket_name,
...
# %%
sentence_data_with_meta = sentence_data.copy()[sentence_data['job id'].isin(job_id_loc_dict)]
...
sentence_data_with_meta['latitude'] = sentence_data_with_meta['job id'].apply(
    lambda x: float(job_id_loc_dict.get(x)[1].split(',')[0]) if job_id_loc_dict.get(x)[1] else None)
sentence_data_with_meta['longitude'] = sentence_data_with_meta['job id'].apply(
    lambda x: float(job_id_loc_dict.get(x)[1].split(',')[1]) if job_id_loc_dict.get(x)[1] else None)
...
# %%
prop_level_a_region = sentence_data_with_meta.groupby('region')['Hierarchy level A name'].apply(
...
plt.savefig('outputs/skills_taxonomy_application/region_application/region_levela_props.pdf',bbox_inches='tight')


# %%
prop_level_a_nuts = sentence_data_with_meta.groupby('NUTs region')['Hierarchy level A name'].apply(
    lambda x: x.value_counts()/len(x))
...
plt.savefig('outputs/skills_taxonomy_application/region_application/nuts_levela_props.pdf',bbox_inches='tight')
...
plt.savefig('outputs/skills_taxonomy_application/region_application/region_levela_props_t.pdf',bbox_inches='tight')



# %%
prop_level_a_nuts.reset_index().groupby(['level_1','NUTs region']).apply(
    lambda x: x['Hierarchy level A name'].iloc[0]).unstack().plot.bar(stacked=False,
                                                                      title ="Proportion of skill types in each region",
                                                                      figsize=(10,4)
                                                                     )
plt.legend(bbox_to_anchor=(1.05, 1))

plt.savefig('outputs/skills_taxonomy_application/region_application/nuts_levela_props_t.pdf',bbox_inches='tight')
...
axes = prop_level_a_nuts.reset_index().boxplot(
    by=['level_1'], column=['Hierarchy level A name'],
    vert=False, figsize=(10,4), color=props,patch_artist=True)
axes.set_title("Spread of proportions of skill types over all NUTs regions");
axes.set_xlabel("");
plt.suptitle("");
plt.savefig('outputs/skills_taxonomy_application/region_application/nuts_levela_props_box.pdf',bbox_inches='tight')
...
level_a_spread = ["Information technology and languages",
                  'Teaching and care',
...
for i, level_a_name in enumerate(level_a_spread):
    ...
    sort_props = df[df['level_1']==level_a_name].set_index('NUTs region').sort_values(by='Hierarchy level A name')
    sort_props.plot.bar(ax=axs[i], title=level_a_name, legend=False, color=color)
    coli+=1
fig.subplots_adjust(hspace=1.3)

plt.savefig('outputs/skills_taxonomy_application/region_application/nuts_levela_props_separate_top.pdf',bbox_inches='tight')
...
coli=0
rowi=0
for i, level_a_name in enumerate(sentence_data_with_meta['Hierarchy level A name'].unique()):
    color = levela_cols[name2num_dict[level_a_name]]
    if i!=0 and i%3==0:
        rowi+=1
        coli=0
    df = prop_level_a_nuts.reset_index()
    sort_props = df[df['level_1']==level_a_name].set_index('NUTs region').sort_values(by='Hierarchy level A name')
    sort_props.plot.bar(ax=axs[rowi,coli], title=level_a_name, legend=False, color=color)
    coli+=1
fig.subplots_adjust(hspace=1.3)
plt.savefig('outputs/skills_taxonomy_application/region_application/nuts_levela_props_separate_all.pdf',bbox_inches='tight')
...
df = pd.concat([df,sentence_data_with_meta.groupby(['NUTs region'])['job id'].nunique()], axis=1)
# Add the proportions of each level A
df = pd.concat([df,
           prop_level_a_nuts.reset_index().pivot(index='NUTs region', columns='level_1', values='Hierarchy level A name')
          ], axis=1)
...
for col_name in sentence_data_with_meta['Hierarchy level A name'].unique():
    df[f"{col_name} - normalised"]=(df[col_name]-df[col_name].min())/(df[col_name].max()-df[col_name].min())
...
coli=0
rowi=0
for i, col_name in enumerate(sentence_data_with_meta['Hierarchy level A name'].unique()):
    ...
    gdf.plot(ax=ax_map[rowi, coli], marker='o', color=color,
             markersize=gdf[f"{col_name} - normalised"]*500, alpha=0.8);
    ax_map[rowi, coli].set_title(f"{col_name}")
    ax_map[rowi, coli].set_axis_off()
    
    coli+=1

plt.savefig('outputs/skills_taxonomy_application/region_application/nuts_levela_props_maps.pdf',bbox_inches='tight')
...
gdf.plot(ax=ax_map[0], marker='o', color=nesta_orange, markersize=gdf['job id']/10, alpha=0.6);
...
plt.savefig('outputs/skills_taxonomy_application/region_application/nuts_numbers_maps.pdf',bbox_inches='tight')
...
plt.savefig('outputs/skills_taxonomy_application/region_application/london_quotient_levb.pdf',bbox_inches='tight')
...
plt.savefig('outputs/skills_taxonomy_application/region_application/london_quotient_leva.pdf',bbox_inches='tight')
