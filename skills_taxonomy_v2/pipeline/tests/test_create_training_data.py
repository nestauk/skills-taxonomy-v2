...
def test_mask_text():
	...
	masked_text = mask_text(nlp, text, spacy_ner_types = ['DATE', 'MONEY', 'CARDINAL', 'TIME', 'ORDINAL', 'QUANTITY'])
...
def test_split_labelled_sentences():
	...
	sentences, sentences_label = split_labelled_sentences(nlp, text, skills_annotations, skill_label_ids=[1])
...
def test_create_training_data():
	...
	jobs_data = [
		{'text': 'hi. this is a skill.', 'annotations': [{'start_offset': 14, 'end_offset': 19, 'label': 1}]},
		{'text': 'and another skill. hello.', 'annotations': [{'start_offset': 12, 'end_offset': 17, 'label': 1}]}
		]
	training_data = create_training_data(nlp, jobs_data, sentence_train_threshold=3, skill_label_ids=[1])
	expected_training_data = [('this is a skill.', 1), ('and another skill.', 1), ('hello.', 0)]

	assert sorted(expected_training_data, key=lambda x: x[0]) == sorted(training_data, key=lambda x: x[0])
